...
class TutorialEnv(SARLevelGen):
    """Compact tutorial: three parts (pickup, unlock, drop) in one room."""
    ...
    def next_part(self):
        self.current_part = min(self.current_part + 1, self.total_parts)
        self.reset()
    ...
    def gen_mission(self):
        ...
        if self.current_part == 1:
            ...
            # create an unlocked door so the agent can open it with the space (toggle) button
            door = Door("red", is_locked=False)
            try:
                door.is_open = False
            except Exception:
                pass
            # put the door at the rightmost grid column (right/gray wall)
            # overwrite whatever is there so the door is actually placed
            try:
                self.grid.set(self.width - 1, cy, door)
            except Exception:
                # fallback to non-overwriting set-if-free
                self._set_if_free(self.width - 1, cy, door)
            # place a matching red key somewhere in the room so the door can be unlocked
            ...
            # ensure the locked door is placed (overwrite if needed)
            try:
                ...
                try:
                    ...
                    pass
                self.grid.set(self.width - 1, cy, d2)
            except Exception:
                self._set_if_free(self.width - 1, cy, Door("red", is_locked=True))
    ...
    def step(self, action):
        ...
        if action == getattr(self.actions, "drop", None):
            if getattr(self, "carrying", None) and getattr(self.carrying, "type", None) == "key":
                ...
                if fwd is not None and self.grid.get(int(fwd[0]), int(fwd[1])) is None:
                    self.grid.set(int(fwd[0]), int(fwd[1]), self.carrying)
                    self.carrying = None
        ...
        if action == getattr(self.actions, "toggle", None):
            ...
            try:
                if fwd is not None:
                    ...
                    if isinstance(obj, MGDoor) and getattr(obj, "is_locked", False):
                        ...
                        if carrying is not None and getattr(carrying, "type", None) == "key" and getattr(carrying, "color", None) == getattr(obj, "color", None):
                            ...
                            try:
                                self.next_part()
                                return self.gen_obs(), 0.0, False, False, {"tutorial_advanced": True}
                            except Exception:
                                pass
                        else:
                            return super().step(action)
                ...
                pass
            # default detect open transition
            pre_open = True
            try:
                from minigrid.core.world_object import Door as MGDoor
                if fwd is not None:
                    o = self.grid.get(int(fwd[0]), int(fwd[1]))
                    pre_open = isinstance(o, MGDoor) and getattr(o, "is_open", False)
            except Exception:
                pre_open = True
            ...
            try:
                if fwd is not None:
                    from minigrid.core.world_object import Door as MGDoor
                    o2 = self.grid.get(int(fwd[0]), int(fwd[1]))
                    post_open = isinstance(o2, MGDoor) and getattr(o2, "is_open", False)
            except Exception:
                post_open = False
    ...
    def get_mission_status(self):
        return {"status": "incomplete", "saved_victims": getattr(self, "saved_victims", 0), "remaining_victims": len(self.get_all_victims())}


class LockedDoorRoom(SARLevelGen):
    """Small room with a locked door on the right (gray wall) and a red key to pick up and use."""
    ...
    def gen_mission(self):
        ...
        # place the matching key in the center so agent must pick it up
        self._set_if_free(cx, cy, Key("red"))

        # minimal instructions surface/verify stubs
                    ...
                    if isinstance(obj, MGDoor) and getattr(obj, "is_locked", False):
                        ...
                        if carrying is not None and getattr(carrying, "type", None) == "key" and getattr(carrying, "color", None) == getattr(obj, "color", None):
                            ...
                            try:
                                ...
                                pass
                            self.carrying = None
                            return self.gen_obs(), 0.0, False, False, {"unlocked": True}
                        else:
                            return super().step(action)
            except Exception:
                pass
